...
class TrainSetLoader(Dataset):
    def __init__(self, dataset_dir, scale_factor, inType='y'):
        ...
        # self.train_list = self.train_list[:10000]
    ...
    def __getitem__(self, idx):
        ...
        for i in range(7):
            try:
                img_hr = Image.open(
                    self.dir + '/sequences/' + self.train_list[idx] + '/im' + str(i + 1) + '.png')
                img_lr = Image.open(
                    self.dir + '/lr_x4/' + self.train_list[idx] + '/im' + str(i + 1) + '.png')
                ...
                print('::::::::::::::::::::Error image img_hr: /sequences/' +
                      self.train_list[idx] + '/im' + str(i + 1) + '.png::::::::::::::::::::')
                print('::::::::::::::::::::Error image img_hr: /lr_x4/' +
                      self.train_list[idx] + '/im' + str(i + 1) + '.png::::::::::::::::::::')
                pass
...
class TestSetLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for idx_frame in range(idx - 3, idx + 4):
            ...
            img_HR = Image.open(self.dataset_dir + '/hr/hr_' +
                                str(idx_frame + 1).rjust(2, '0') + '.png')
            img_LR = Image.open(
                self.dataset_dir + '/lr_x4/lr_' + str(idx_frame + 1).rjust(2, '0') + '.png')
        ...
        C, N, H, W = HR.shape
        H = math.floor(H / self.scale_factor / 4) * self.scale_factor * 4
        W = math.floor(W / self.scale_factor / 4) * self.scale_factor * 4
        HR = HR[:, :, :H, :W]
        SR_buicbic = SR_buicbic[:, :H, :W]
        LR = LR[:, :, :H // self.scale_factor, :W // self.scale_factor]
    ...
    def __getitem__(self, idx):
        ...
        for idx_frame in range(idx - 3, idx + 4):
            ...
            if idx_frame == idx:
                ...
                SR_buicbic = np.array(
                    img_LR_o.resize(
                        (w * self.scale_factor, h * self.scale_factor), Image.BICUBIC),
    ...
    def __getitem__(self, idx):
        ...
        for idx_frame in range(idx - 3, idx + 4):
            ...
            if idx_frame > len(self.img_list) - 1:
                idx_frame = len(self.img_list) - 1
            img_hr = Image.open(
                self.dir + '/sequences/' + self.video_name + '/im' + str(idx_frame + 1) + '.png')
            img_lr = Image.open(
                self.dir + '/lr_x4/' + self.video_name + '/im' + str(idx_frame + 1) + '.png')
            img_hr = np.array(img_hr, dtype=np.float32) / 255.0
            if idx_frame == idx:
                h, w, c = img_hr.shape
                SR_buicbic = np.array(img_lr.resize(
